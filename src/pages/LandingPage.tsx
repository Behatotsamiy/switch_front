...
export const LandingPage: React.FC = () => {
  ...
  // Плавный скролл к разделу мероприятий
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 py-12 md:py-16 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight text-slate-900 dark:text-white">
            ...
            <span className="text-purple-600 dark:text-purple-400">{t.landing?.heroTitle2 || 'SWITCH Community'}</span>
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <button
              ...
              className="px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-full shadow-lg shadow-purple-500/20 active:scale-[0.98] transition flex items-center gap-2"
            >
              {isAuthenticated ? (
                <>
            ...
            <button
              ...
              className="px-6 py-3 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 font-semibold rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/60 transition"
            >
          ...
          {[
            ...
            >
              ...
              <div className="p-6 pt-0">
                <button
                  onClick={handlePrimaryAction}
                  className="w-full py-2.5 bg-purple-50 dark:bg-slate-800 text-purple-700 dark:text-purple-300 font-semibold rounded-xl hover:bg-purple-100 dark:hover:bg-slate-700 transition text-sm"
                >
                  {isAuthenticated ? (t.landing?.goToProfile || 'Перейти в личный кабинет') : (t.landing?.viewDetails || 'Подробнее')}
                </button>
              </div>
            </div>
          ))}
